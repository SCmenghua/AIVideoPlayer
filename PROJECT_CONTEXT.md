...
**Phase 7.x 系列**（进行中）：在 Phase 8 之前用 7.x 的数个小版本修好播放器功能。
**Phase 8**（规划中）：修好语音识别功能与翻译功能（二者当前仍为半成品状态）。
**Phase 9 & Phase 9+**（规划中）：完成 Liquid Glass 深化（变形过渡）、性能、
测试与错误处理。
上一阶段：**Phase 7.9 —— 播放器细节优化 + 主页标签页**（已完成；Phase 7.5–7.9
已整体收尾）。
当前阶段：**Phase 7.10 —— 横屏识别/方向切换修复 + 媒体来源**（进行中）。
...
- **版本号与打包约定**：同一 Phase 内按小版本迭代时（如 Phase 7.x 系列），
  ...
  当前基线：0.7.9（Phase 7.9），下一次打包应为 0.7.10（Phase 7.10）。
