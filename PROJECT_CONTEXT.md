...
- **Phase 9.5（2026-08-13 打包 0.9.5）**：修复长视频等待门控——识别停滞即触发首批——
  ...
  识别撞上前瞻上限时立即把已识别内容打包发送，不再依赖凑满 60s 语音段；`handlePlaybackEnded`
  重置批处理协调器与 `initialBatchCompleted`，修复重新初始化后门控状态残留；
  保持「首批翻译返回后再播放」；`MARKETING_VERSION` 定为 0.9.5。
...
- **Phase 8.18（2026-08-13 打包 0.8.18）**：修复长视频（10 分钟以上）开启字幕 + 翻译后 UI 卡死——
  ...
  而跳回主线程），配合 `PCMBuffer` 每次超出上限都用 `Array.removeFirst`（O(n)）整体搬移，
  长视频下主线程被近平方级的 memmove 拖垮；修复：`PCMBuffer` 改为「存储数组 + 头下标」环形窗口，
  裁剪只推进头下标并周期性压实（均摊 O(1)）；`AudioPipeline` 新增 `setReadTarget`，AssetReader
  只预读播放位置前方 30 秒（由字幕管线随播放进度推进），不再整轨解码；`updatePlaybackPosition` /
  `preparePlayback` / `handleSeek` 同步推进预读目标。新增「诊断与日志」二级菜单（设置页进入），
  集中展示字幕记录 / 翻译记录 / 日志，三者独立开关；`AppLogger` 重构为内存环形缓冲（500 条）
  + 后台异步落盘、去掉逐行 fsync，并展示日志列表（级别着色 / 导出 / 清空）；重做识别与翻译日志：
  识别窗口完成日志含耗时 / 采样数 / 语言 / 段数，翻译日志含 provider / 源目标语言 / 耗时，
  跳过落后窗口日志含累计次数与秒数；swift-ci 构建 + 单元测试全绿；`MARKETING_VERSION` 提升至 0.8.18。
