...
**Phase 8.10**（已完成，2026-08-11 打包 0.8.10）：修复字幕开启后 UI 卡住——
`SubtitlePipeline.forwardSegment()` 改为 `nonisolated` 并通过 `Task { @MainActor }`
异步写入字幕记录，避免字幕高频产生时（尤其是 partial 段）同步阻塞主线程导致
UI 冻结（界面无响应但音频继续播放）；`MARKETING_VERSION` 提升至 0.8.10。
...
当前代码基线：0.8.10（Phase 8.10，2026-08-11；0.8.2 – 0.8.4 与 0.8.8 全部失败弃用）。

## 已完成

- **Phase 8.10（2026-08-11 打包 0.8.10）**：修复字幕开启后 UI 卡住——
  `SubtitlePipeline.forwardSegment()` 改为 `nonisolated` 并通过 `Task { @MainActor }`
  异步写入字幕记录，避免字幕高频产生时（尤其是 partial 段）同步阻塞主线程导致
  UI 冻结（界面无响应但音频继续播放）；`MARKETING_VERSION` 提升至 0.8.10。
