...
**Phase 8.4**（进行中，2026-08-10 开启）：因 0.8.2 / 0.8.3 实测仍存在字幕回归，
代码基线回退至 Phase 8.1 / 0.8.1（commit `4449b16`），保留说明文档并记录本次回退；
继续修复用户实测反馈的剩余 bug（具体清单待用户提供）。
...
- **版本号与打包约定**：同一 Phase 内按小版本迭代时（如 Phase 7.x 系列），
  ...
  当前代码基线：0.8.1（Phase 8.1，2026-08-10 由 0.8.3 回退至此），
  下一次打包应为 0.8.4（Phase 8.4）。
