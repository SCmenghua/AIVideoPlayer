...
字幕采用实时识别设计：Whisper 按 partial → final 输出，final 后即时翻译，
字幕按整句收敛并支持拖动与样式调整；设置页提供「字幕记录」查看已识别原文与译文，
便于排查字幕问题（详见下文「AI 实时字幕」）。
...
- 单元测试（Swift Testing）：模型、Mock 数据、WebDAV 解析、存储、浏览器与远程文件 ViewModel、
  AI 字幕管线（PCM 缓冲游标 / 实时路径 partial → final / seek 重建 / 播放器联动）、协议测试替身
...
- 播放器联动：播放 / seek 前重建并启动音频来源、暂停停止识别、识别游标只进不退、
  落后窗口自动跳过；浏览器首页 AI 状态卡与播放器共用同一管线
...
- 字幕语言（Phase 8.6）：设置页独立「字幕语言」卡片——原语言（自动检测 +
  简体中文 / English / 日本語 / 한국어 / Bahasa Melayu / Filipino / ไทย /
  Tiếng Việt / Bahasa Indonesia / Français / Deutsch / Español 共 12 种）、
  翻译语言（同样 12 种）、双语显示开关；手动指定源语言后 Whisper 识别与翻译
  源语言立即生效，自动检测时跟随识别结果；切换语言会重建翻译引擎
- 双语显示（Phase 8.6）：开启时字幕上行原文（小字号，约为译文一半）、
  下行译文（主行大字号）；关闭时只显示译文一行（译文缺失时显示原文）；
  开关持久化，默认开启
- 共享字幕记录（Phase 8.5）：`SubtitleTranscriptStore` 有界保留最近 200 条已识别字幕
  （原文 + 译文），播放器叠加层与设置页「字幕记录」卡片直接读取——不再依赖单次消费
  的流，Tab 反复进出 / 全屏切换后字幕不会丢失；final 到达自动收敛逐词残留；
  普通播放与全屏播放共享同一叠加层
...
- 字幕翻译接入（Phase 7 → 8.7）：final 段识别后立即翻译并写入译文；
  翻译默认开启（本地 Fast NMT）；翻译失败时只显示原文并在设置页提示原因；
  翻译期间状态卡显示 TRANSLATING；设置页识别状态行显示「已翻译 N 条」
- 翻译记录（Phase 8.7）：设置页独立「翻译记录」卡片——展示最近 20 条
  已成功翻译的字幕（原文小字 + 译文大字 + 时间），头部显示已翻译总数，
  翻译失败时直接展示失败原因；与「字幕记录」共享同一数据源
- 设置页「翻译服务」卡片（Phase 7）：Provider 选择（系统翻译 / 本地大模型 /
  云端 API）、本地模型下载管理（Gemma 4 E2B 按需下载）、云端配置与测试连接、
  启用校验；目标语言已移至「字幕语言」卡片（Phase 8.6）
...
1. **Package IPA (unsigned)**（推荐日常测试）：构建未签名 IPA，同时上传为
   Actions 工件并发布到 GitHub Release（可填版本号，默认 0.8.11）；
2. **Build & Release IPA**：构建未签名 IPA 并创建 GitHub Release
   （`AIVideoPlayer-<版本>-unsigned.ipa`，可填版本号，默认 0.8.11）。
...
当前基线 0.8.7（Phase 8.7，2026-08-10；0.8.2–0.8.4 与 0.8.8 全部失败弃用）。

> ⚠️ **Phase 8.2 – Phase 8.4 全部失败（2026-08-10 标记）**：实测无法正常使用，
> 实现已回退弃用；后续所有 Phase 将忽略这几次代码，未来从 Phase 8.1 / 0.8.1
> 代码开始重构，下一次 Phase 为 8.5。

> ⚠️ **Phase 8.8 失败（2026-08-11 标记）**：修好了大视频播放 bug，但网页视频仍
> 无法播放，且字幕功能回归不可用（卡片显示错误 / 停在「正在聆听」、字幕记录与
> 翻译记录为空）；代码已回退至 0.8.7 基线弃用，后续从 Phase 8.7 / 0.8.7 重构，
> 下一次 Phase 为 8.9。
...
- ✅ Phase 5 完成：WhisperKit 实时识别（AudioPipeline / SpeechRecognizer / 真实状态管线；
  模型内置；固定 5 秒窗口 partial → final），CI 通过
- ✅ Phase 6 完成：SubtitleOverlay 双语整句字幕叠加（按播放光标对齐、拖动、样式；
  SubtitleTranscriptStore / SubtitleDisplaySettings / 单元测试），CI 通过
...
- ✅ Phase 8.1 完成（打包 0.8.1）：修复「开关已生效但看不到字幕」——确认开关链路真实生效；
  修正识别参数（标准 prefill + 语言传递）、识别游标跟随播放并跳过落后窗口；
  设置页新增识别状态统计（模型加载 / 转写窗口 / 字幕产出）
- ✅ Phase 8.5 完成（打包 0.8.5）：删除超前识别功能；重构字幕显示链路——新增共享
  `SubtitleTranscriptStore`，播放器 Overlay 与设置页直接读取（不再消费单次迭代的流，
  修复 Tab 反复进出后字幕丢失）；实时路径 partial 不再按播放位置丢弃；
  设置页新增「字幕记录」卡片查看已识别字幕原文 + 译文并支持清空
- ✅ Phase 8.6 完成（打包 0.8.6）：新增字幕语言与双语显示——设置页新增独立
  「字幕语言」卡片（原语言自动检测 + 12 种 / 翻译语言 12 种 / 双语显示开关）；
  手动指定源语言后 Whisper 识别与翻译源语言立即生效，切换语言重建翻译引擎；
  双语显示开启时上行原文（约译文一半）、下行译文，关闭时只显示译文
- ✅ Phase 8.7 完成（打包 0.8.7）：修复「只显示原文、不显示译文」——根因是
  翻译默认关闭导致 final 段从不调用翻译引擎；现在默认开启（本地 Fast NMT），
  翻译成功条数 / 失败原因可观测（设置页显示 + OSLog），零时长 final 兜底，
  新增设置页独立「翻译记录」卡片（原文 + 译文 + 时间 + 失败提示）
- ❌ Phase 8.8 失败（打包 0.8.8，代码已弃用）：修复大视频 UI 冻结（读取器解码
  移出主线程）与网络资源播放停摆（本地才用读取器、网络走 Tap、HLS 跳过采集）；
  实测修好大视频 bug，但网页视频仍无法播放，且字幕功能回归不可用（卡片错误 /
  停在「正在聆听」、字幕记录与翻译记录为空），已回退至 0.8.7 基线
...
- ❌ Phase 8.3 失败（打包 0.8.3，代码已弃用）：实测反馈修复——播放器字幕开关随识别管线
  自动同步、实时字幕显示修复、设置页语言与 Provider 选择器交互修复、
  语言菜单分栏标注原语言 / 目标语言；实测仍无法使用，代码已回退弃用
- ❌ Phase 8.4 失败（打包 0.8.4，代码已弃用）：在 0.8.1 基线上重建多语言与源/目标语言——
  全语言池 12 种（含日文等），设置页可勾选呈现并排序；设置页与播放器均可调整
  「原语言 / 目标语言」；播放器字幕语言按钮紧邻字幕开关、开启字幕后才出现，
  菜单分栏明确标注原语言 / 目标语言（0.8.2/0.8.3 实现曾因字幕回归回退）；
  2026-08-10 实测仍无法使用，结束本次 Phase 并二次回退至 0.8.1 基线
...
| 5 | WhisperKit 本地实时识别（AudioPipeline + SpeechRecognizer；固定 5 秒窗口 partial → final；模型内置） | ✅ 完成 |
| 6 | SubtitleOverlay（双语、整句按播放光标对齐一次性出现、拖动、样式） | ✅ 完成 |
| 7 | TranslationEngine：Fast NMT（Apple 原生翻译）/ 本地 LLM（MLX Swift + Gemma 4 E2B，按需下载）/ 云端 API（OpenAI 兼容，测试连接 + 隐私提示）+ 剧情理解润色（自动压缩文本）；识别后立即翻译 | ✅ 完成 |
...
| 8.2 | 翻译功能增强：系统翻译 / 本地大模型 / 云端 API、原语言与目标语言、多语言池 12 种可配置、播放器语言按钮（打包 0.8.2） | ❌ 失败（弃用） |
| 8.3 | 实测反馈修复：播放器字幕开关同步、实时字幕显示、设置页选择器交互、语言菜单分栏（打包 0.8.3） | ❌ 失败（弃用） |
| 8.4 | 多语言与源/目标语言重建（0.8.1 基线；打包 0.8.4） | ❌ 失败（弃用） |
...
| 8.8 | 修复播放器两个 bug：大视频 UI 冻结（读取器解码移出主线程）、网络资源播放停摆（本地才用读取器、网络走 Tap、HLS 跳过采集）（打包 0.8.8） | ❌ 失败（弃用） |
| 9 & 9+ | Liquid Glass 深化（变形过渡）、性能、测试与错误处理 | ⬜ 未开始 |

#### AI 实时字幕（实时路径）

实时字幕走 partial → final 路径：固定 5 秒识别窗口，Whisper 逐词输出 partial，
final 到达后收敛为整句并即时翻译；字幕仍按播放光标对齐，双语显示时
整句上行原文（小字）、下行译文（大字），关闭双语时只显示译文。
识别游标只进不退、落后播放光标的窗口自动跳过；seek / 暂停恢复后丢弃过期结果。
设置页「字幕记录」卡片直接展示已识别字幕的原文 + 译文 + 时间，可一键清空，
用于排查「识别已产出但播放器无字幕」等显示问题。

- Phase 8.6 起字幕语言可在设置页「字幕语言」卡片调整：原语言（视频语音，自动检测 +
  12 种语言）决定 Whisper 识别语言与翻译源语言；翻译语言（12 种）决定译文输出语言；
  双语显示开关控制字幕两行 / 单行显示。
