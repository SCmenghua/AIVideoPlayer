...
1. **Package IPA (unsigned)**（推荐日常测试）：构建未签名 IPA，同时上传为
   Actions 工件并发布到 GitHub Release（可填版本号，默认 0.7.11）；
2. **Build & Release IPA**：构建未签名 IPA 并创建 GitHub Release
   （`AIVideoPlayer-<版本>-unsigned.ipa`，可填版本号，默认 0.7.11）。
...
（`Phase 7.x` → `Phase 7.(x+1)`）。当前基线 0.7.11，下一次打包应为 0.7.12。
...
- 🔄 Phase 7.x 系列进行中：Phase 8 之前用 7.x 的数个小版本修好播放器功能
  （当前基线 0.7.11 / Phase 7.11，下一次打包 0.7.12 / Phase 7.12）
...
| 7.12+（Phase 8 之前） | 继续用 7.x 的数个小版本修好播放器功能（每次打包版本递增 0.7.x → 0.7.(x+1)，Phase 编号同步） | 🔄 进行中 |
