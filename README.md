...
- 设置页「翻译服务」卡片（Phase 7 / 8.1）：Provider 选择（系统翻译 / 本地大模型 /
  云端 API）、原语言与目标语言（语言池 12 种：中 / 英 / 日 / 韩 / 马来 / 菲律宾 /
  泰 / 越 / 印尼 / 法 / 德 / 西，可勾选呈现并排序）、本地模型下载管理
  （Gemma 4 E2B 按需下载）、云端配置与测试连接、启用校验；播放器字幕开启后
  控制栏提供「字幕语言」快捷按钮
...
1. **Package IPA (unsigned)**（推荐日常测试）：构建未签名 IPA，同时上传为
   Actions 工件并发布到 GitHub Release（可填版本号，默认 0.8.5）；
2. **Build & Release IPA**：构建未签名 IPA 并创建 GitHub Release
   （`AIVideoPlayer-<版本>-unsigned.ipa`，可填版本号，默认 0.8.5）。
...
当前基线 0.8.5（Phase 8.5，2026-08-10 重构；0.8.2–0.8.4 全部失败弃用）。
...
final 到达后收敛为整句并即时翻译；字幕仍按播放光标对齐，整句显示原文 + 译文。
