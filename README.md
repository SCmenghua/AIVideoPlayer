...
- 字幕翻译接入（Phase 7）：final 段识别后立即翻译并写入译文；
...
1. **Package IPA (unsigned)**（推荐日常测试）：构建未签名 IPA，同时上传为
   Actions 工件并发布到 GitHub Release（可填版本号，默认 0.8.6）；
2. **Build & Release IPA**：构建未签名 IPA 并创建 GitHub Release
   （`AIVideoPlayer-<版本>-unsigned.ipa`，可填版本号，默认 0.8.6）。
...
当前基线 0.8.6（Phase 8.6，2026-08-10；0.8.2–0.8.4 全部失败弃用）。
