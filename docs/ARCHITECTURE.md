...
- 识别参数与游标（Phase 8.1）：转写使用标准 prefill（SOT + 语言 + 任务 + 时间戳），
  首窗自动检测语言后把语言代码传给后续窗口（避免中文 / 短音频逐窗检测失败）；
  播放器进度实时推送给管线，识别速度跟不上播放时跳过已落后的窗口、从当前播放
  位置继续；设置页展示识别状态统计（模型加载 / 转写窗口 / 字幕产出），
  关键链路有 OSLog 日志，便于确认识别是否真的被调用。
...
19. **Phase 8.1（已完成，打包 0.8.1）**：修复「开关已生效但看不到字幕」——
    ...
    设置页识别状态统计。
20. **Phase 8.2（已完成，打包 0.8.2）**：翻译功能增强——系统翻译 / 本地大模型
    （Gemma 4 E2B 按需下载）/ 云端 API 可选；原语言与目标语言选择；多语言池
    12 种、设置页可勾选呈现并排序；播放器字幕开启后出现「字幕语言」按钮。
21. **Phase 8.3（已完结，打包 0.8.3）**：实测反馈修复——播放器字幕开关随识别
    ...
    仍有实测 bug，Phase 8.4 继续修复。
22. **Phase 8.4（进行中，打包 0.8.4）**：在 0.8.1 基线上重建多语言与源/目标语言——
    全语言池 12 种（含日文等），设置页可勾选呈现并排序；设置页与播放器均可调整
    「原语言 / 目标语言」；播放器「字幕语言」按钮紧邻字幕开关、开启字幕后才出现，
    菜单分栏明确标注原语言 / 目标语言。继续修复用户实测反馈的剩余 bug。
23. **Phase 9 & Phase 9+（规划中）**：完成 Liquid Glass 深化（变形过渡）、性能、
    测试与错误处理。
