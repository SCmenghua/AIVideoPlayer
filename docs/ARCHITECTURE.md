...
36. **Phase 8.18（已完成，打包 0.8.18）**：修复长视频（10 分钟以上）开启字幕 + 翻译后
    ...
    更有意义的识别 / 翻译日志。
37. **Phase 9.7（已完成，版本 0.9.7）**：优化 Whisper 字幕句段输出——本地预加载视频使用 10 秒
    识别窗口，HLS / 实时播放保持 5 秒；同一窗口内合并相邻的 final 段，受时间间隔、句末标点、
    8 秒时长和 120 字符上限约束，partial 段保持不变；识别日志同时记录合并后与原始段数。
38. **Phase 9.8 及后续（规划中）**：继续处理字幕连续性与翻译质量，并推进 Liquid Glass 深化、性能、
    测试与错误处理。
