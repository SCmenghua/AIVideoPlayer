...
> **项目状态：已停止并归档。** 本文件仅用于理解旧实现、复盘问题和复现历史构建。本仓库不再进行新的 Phase、功能开发、问题修复或 IPA 发布；文中的“后续”“规划中”和“未来接入”均已失效。
...
#### 8.2.1 实时识别路径（Phase 8.5 移除超前识别；Phase 9.8-9.10 稳定性保证）
...
- Phase 9.10 的质量门控覆盖输入与输出：语音窗口必须有足够的帧间能量变化，避免恒定底噪进入
  Whisper；解码显式使用低平均概率、首 token、压缩比和无语音阈值；final 结果由
  `SpeechRecognitionQualityGate` 统一处理，低置信度或 8 秒内重复的归一化文本不写入字幕历史、
  翻译队列或翻译记录。规则与语言、具体词句无关，切换识别会话时会重置。
...
**Phase 9.10** 补充输入、解码与 final 输出三层质量门控，用于抑制低信息量音频的重复幻觉。
...
5. `SubtitlePipeline` 接入：final 段先写入原文，再异步按 ID 回填 `translatedText`；
   ...
   （`lastTranslationError`），翻译开始 / 成功 / 失败均有 OSLog，便于确认
   「没有调用翻译」还是「翻译失败 / 显示问题」。
...
7. 设置页「字幕语言」卡片（`SubtitleLanguageCard`，Phase 8.6，与字幕记录同层级）：
   ...
   保证下次识别 / 翻译立即按新选择生效。
8. 设置页「翻译记录」卡片（`SubtitleTranslationCard`，Phase 8.7）：
   展示最近 20 条已成功翻译的字幕（原文 + 译文 + 时间），头部显示已翻译总数，
   翻译失败时展示 `lastTranslationError`；与「字幕记录」共享同一数据源，
   用于确认系统翻译是否真的被调用并产出译文。
...
25. **Phase 8.7（已完成，打包 0.8.7）**：修复「只显示原文、不显示译文」——
    根因是翻译默认关闭导致 final 段从不调用翻译引擎；修复为默认开启（本地
    Fast NMT），管线增加翻译计数与失败原因（设置页显示 + OSLog），
    零 / 负时长 final 按 0.5s 最小窗口兜底；新增设置页独立「翻译记录」卡片
    （原文 + 译文 + 时间 + 已翻译总数 + 失败提示）。
26. **Phase 8.8（❌ 失败，打包 0.8.8，代码已弃用）**：修复两个播放器 bug——
    大视频 UI 冻结（AVAssetReader 解码循环移出主线程）、网络资源播放停摆
    （本地文件才用读取器、网络媒体走实时 Tap、HLS 跳过音频采集）；实测修好了
...
38. **Phase 9.10（已完成，0.9.10）**：新增输入能量变化、WhisperKit 解码阈值、final
    低置信度与短间隔重复三层质量门控，抑制背景噪声引起的重复幻觉字幕而不硬编码任何文本。
39. **Phase 9 & Phase 9+（历史规划，未继续实施）**：曾记录 Liquid Glass 深化（变形过渡）、
    性能、测试与错误处理方向；项目停止后不再作为开发计划。

> 本项目已停止并归档。以上 Phase 记录仅供历史复盘，变更记录见 [CHANGELOG.md](../CHANGELOG.md)。
