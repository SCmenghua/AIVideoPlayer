...
- **后续**：LLM 提示词与推理性能优化、更多模型、浏览器功能和 Liquid Glass 深化
- **后续**：新浏览器功能、Liquid Glass 深化、性能优化与测试（编号后续规划）

## [0.9.7] - 2026-08-13

### Changed（Phase 9.7 Whisper 句段收敛）
- 确认系统内置翻译使用 Apple Translation Framework；先优化 Whisper 输出，减少碎片化文本对翻译质量的影响。
- 本地预加载视频的 Whisper 识别窗口由 5 秒扩大到 10 秒；HLS / 实时播放保持 5 秒以控制延迟。
- 在同一识别窗口内合并相邻的 final 段：时间间隔需在 -0.5 至 0.5 秒、前段无句末标点，合并结果最长 8 秒、120 字符。
- Latin 文本保留单词间空格，CJK 文本不额外插入空格；partial 段保持原样，不参与合并。
- 识别日志同时记录合并后的 `segments` 与原始 `rawSegments` 数量，新增句段合并单元测试。
- `MARKETING_VERSION` 定为 0.9.7；Windows 无法运行 iOS 测试，CI 待推送后验证。
