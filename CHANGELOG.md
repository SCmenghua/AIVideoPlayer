...
### Fixed（Phase 9.10 Whisper 幻觉字幕抑制）

- 修复低信息量背景声、持续底噪或音频伪影被送入 Whisper 后，反复产出不存在字幕的问题；
  问题发生在识别阶段，翻译引擎只会正常翻译错误原文。
- `SpeechWindowPlanner` 新增帧间能量变化检测：恒定能量的背景音不再仅因超过 RMS 阈值就进入转写。
- `WhisperKitSpeechRecognizer` 显式启用低平均 log probability、首 token 置信度、压缩比和无语音
  回退阈值，优先由 WhisperKit 拒绝低质量、重复和疑似静音窗口。
- 新增通用 `SpeechRecognitionQualityGate`：final 字幕低置信度直接丢弃；同一归一化文本在 8 秒内
  重复出现时不写入字幕历史、翻译队列或翻译记录。规则不依赖任何语言或特定短语，真实台词可在
  窗口之外或新会话中正常出现。

### Changed（Phase 9.10）

- 新增背景恒定音、低置信度 final、短间隔重复字幕、新会话重置的回归测试。
- `MARKETING_VERSION` 提升至 0.9.10，两个 IPA 工作流默认版本同步更新。

## [0.9.9] - 2026-08-14

### Fixed（Phase 9.9 按语音停顿划分字幕识别窗口）

- 修复实时字幕识别输入被固定 5 秒切片的问题；新增 `SpeechWindowPlanner`，以 100ms PCM 能量帧识别语音和静音
- 跳过前导静音与纯静音，不再把无声音频送入 Whisper；连续语音达到至少 0.4s 后，检测到 0.5s 尾部静音即结束当前识别窗口
- 无停顿的连续讲话采用 8s 上限兜底，避免无限等待；WhisperKit 仍负责输出最终字幕段和时间戳
- 落后播放光标超过容忍范围时直接重同步到当前播放位置，不再依赖固定 5 秒窗口边界

### Changed（Phase 9.9）

- 新增语音窗口分段器单元测试，以及字幕管线集成测试，覆盖停顿截断、静音跳过、连续语音兜底和缓冲等待
- `MARKETING_VERSION` 提升至 0.9.9，IPA 工作流默认版本同步更新

## [0.9.8] - 2026-08-14

### Fixed（Phase 9.8 长视频实时字幕稳定性修复）
...
- 识别游标允许最多落后 10 秒；只有超过阈值才重同步到当前播放位置，减少频繁跳过音频

### Changed（Phase 9.8 长视频实时字幕稳定性修复）
...
- `MARKETING_VERSION` 提升至 0.9.8；原 `AIVideoPlayer-0.9.6-unsigned` IPA 与 Release 保持不变
...
- 识别撞上前瞻上限（ahead-wait）时立即 flush 首批，不再等凑满 60s 语音段
- 重新初始化 / 播放结束后重置批处理与门控状态，避免状态残留

### Changed（Phase 9.5）

- `MARKETING_VERSION` 定为 0.9.5

## [0.9.4] - 2026-08-13

### Fixed（Phase 9.4 修复等待门控卡死）

- 短视频（< 60s）不再因首批窗口凑不满而卡死在「等待大模型返回结果」
- 识别停滞时立即 flush 已识别内容并完成首批

### Changed（Phase 9.4）

- 首批批量翻译失败改为重排队列 + 退避重试，等待期间不放行播放、不显示原文
- 批量翻译新增日志：发送条数 / 收到回复条数与耗时 / 失败原因
